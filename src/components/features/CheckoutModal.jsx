...
`;
            ...
            console.error(err);
            setError("Ошибка инициализации оплаты. Проверьте консоль или попробуйте позже.");
            setStep('form');
        }
    };
            ...
            <div className="bg-white dark:bg-stone-900 w-full sm:w-[400px] rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl animate-slide-up flex flex-col max-h-[90vh]">
                <div className="p-4 flex justify-between items-center border-b border-stone-100 dark:border-stone-800 shrink-0">
                    <h3 className="font-serif font-bold text-lg text-stone-800 dark:text-stone-100">
                        {step === 'form' ? 'Оформление заказа' : (step === 'success' ? 'Готово!' : 'Оплата')}
                    </h3>
                    {step !== 'processing' && (
                        <button onClick={onClose} className="p-1 hover:bg-stone-100 dark:hover:bg-stone-800 rounded-full">
                ...
                {step === 'form' && (
                    <div className="overflow-y-auto p-6 custom-scrollbar">
                        ...
                        <div className="mb-6 space-y-2">
                            {cartItems.map(item => (
                                <div key={item.id} className="flex justify-between text-sm">
                                    <span className="text-stone-600 dark:text-stone-400 truncate max-w-[200px]">{item.name} x{item.quantity}</span>
                                    <span className="font-medium dark:text-stone-200">{(item.price * item.quantity).toLocaleString()} ₽</span>
                                </div>
                            ))}
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <button type="submit" className="w-full bg-stone-800 dark:bg-primary-600 text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 mt-4 hover:bg-stone-700 dark:hover:bg-primary-500 transition-colors">
                                <CreditCard size={18} />
                ...
                {step === 'processing' && (
                    <div className="p-12 text-center">
                        <Loader2 size={32} className="text-stone-800 dark:text-stone-100 animate-spin mx-auto mb-4" />
                        <h3 className="font-bold text-stone-800 dark:text-stone-100">Обработка платежа...</h3>
                        <p className="text-stone-500 dark:text-stone-400 text-sm mt-2">Пожалуйста, не закрывайте окно</p>
                    </div>
                )}
                ...
                {step === 'success' && (
                    <div className="p-8 text-center">
                        ...
                        <h3 className="font-bold text-xl mb-2 dark:text-stone-100">Заказ оформлен!</h3>
                        <p className="text-stone-500 dark:text-stone-400 text-sm mb-6">
                            Мы свяжемся с вами в ближайшее время для подтверждения деталей.
                        </p>
                        <button onClick={onClose} className="w-full bg-stone-800 dark:bg-primary-600 text-white py-3 rounded-xl font-bold hover:bg-stone-700 dark:hover:bg-primary-500 transition-colors">
                            Отлично
                        </button>
                    </div>
                )}
