...
const AIChat = () => {
    const [messages, setMessages] = useState([
        { text: "Здравствуйте! Я Мастер Арбареа. 🌿 Чем могу помочь? Рассказать про дубовые столы или подобрать панно?", sender: 'ai' }
    ]);
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            setMessages(prev => [...prev, { text: "Произошла ошибка. Попробуйте позже.", sender: 'ai' }]);
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pb-4 custom-scrollbar">
                {messages.map((m, i) => (
                    <div key={i} className={`flex gap-3 ${m.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${m.sender === 'user' ? 'bg-stone-800 text-white' : 'bg-white border border-stone-200 text-stone-600'}`}>
                            {m.sender === 'user' ? <User size={16} /> : <Bot size={16} />}
                        </div>
                        <div className={`p-4 rounded-2xl max-w-[80%] text-sm leading-relaxed shadow-sm ${m.sender === 'user' ? 'bg-stone-800 text-white rounded-tr-none' : 'bg-white border border-stone-100 text-stone-700 rounded-tl-none'}`}>
                            {m.text}
                        </div>
                    </div>
                ))}
                {isLoading && (
                    <div className="flex gap-3">
                        <div className="w-8 h-8 rounded-full bg-white border border-stone-200 flex items-center justify-center text-stone-600"><Bot size={16} /></div>
